...
PANDOC/CROSSREF := docker run -v "`pwd`:/data" --user "`id -u`:`id -g`" pandoc/crossref:2.11.0.4
PANDOC/LATEX := docker run -v "`pwd`:/data" -v "`pwd`/assets/fonts:/usr/share/fonts" --user "`id -u`:`id -g`" pandoc/latex:2.11.0.4
